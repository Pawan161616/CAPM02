...
//In below code "Redirected to" is for association, but it's not needed ,we can directly use association with $expand on above entities.
    entity POs @(
        title: '{i18n>poHeader}',
        //below line is needed for posting data to backend
        //below line enables creation of transient/draft table which stores data before posting to database successfully.
